...
class MachineApp {
  ...
  _processLlmResponse = (llmResponseData, originalCmjMessages) => {
    try {
      ...
      if (!Array.isArray(llmResponseData)) {
        console.error("Expected an array of response objects.");
      }
      
      for (const item of llmResponseData) {
        if (item && Array.isArray(item.content)) {
          for (const chunk of item.content) {
            // Check for reasoning text
            if (chunk.type === 'reasoning_text' && typeof chunk.text === 'string') {
              thoughtsText += chunk.text;
            }
            // Check for output text (also fallback to 'text' type if used by some APIs)
            else if ((chunk.type === 'output_text' || chunk.type === 'text') && typeof chunk.text === 'string') {
              regularText += chunk.text;
            }
          }
        }
      }
